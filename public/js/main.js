document.addEventListener('DOMContentLoaded', function() {
  ...
  // ==========================================
  const unconsentedCheck = document.getElementById('unconsented_check');
  const witnessContainer = document.getElementById('witness_signature_container');
  
  const consentFields = [
    { id: 'consent_date', labelContainer: null },
    { id: 'signature_assessed', labelContainer: null },
    { id: 'witness_signature', labelContainer: witnessContainer }
  ];

  function updateConsentLogic() {
    const isUnconsented = unconsentedCheck.checked;
  ...
  if (unconsentedCheck) {
    unconsentedCheck.addEventListener('change', updateConsentLogic);
  ...
  const radioButtons = document.getElementsByName('add_phone_2');
  ...
  dateInputs.forEach(input => {
    input.addEventListener('input', function(e) {
      ...
      // Limit to 8 digits maximum (DD MM YYYY)
      if (v.length > 8) v = v.substring(0, 8); 
      
      // Automatically add the slashes
      let formatted = v;
      if (v.length > 4) {
  ...
  const personNameDisplay = document.getElementById('person_name_display');
  ...
  function updatePersonName() {
    if (personNameDisplay) {
      ...
      personNameDisplay.value = `${first} ${last}`.trim();
    }
  }
